...
export function StaffVisibilityView() {
  ...
  const identityRetained = subject?.identityAssociation === "track_session";
  const identityDetail = subject?.employeeId
    ? identityRetained
      ? `Identity retained on Track ${subject.trackId ?? "—"}; badge ${subject.badgeMarkerId ?? "—"} verified ${formatRelative(subject.identityVerifiedAt ?? null)}.`
      : `Badge ${subject.badgeMarkerId ?? "—"} is visible and verifies Track ${subject.trackId ?? "—"}.`
    : "Identity optional; PPE remains active";
