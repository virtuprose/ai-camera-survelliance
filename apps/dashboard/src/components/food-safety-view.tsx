...
export function FoodSafetyView() {
  ...
  const identityRetained = employee?.identityAssociation === "track_session";
  const identityDetail = employee?.employeeId
    ? identityRetained
      ? `Identity retained on Track ${employee.trackId ?? "—"}; badge ${employee.badgeMarkerId ?? "—"} verified ${formatRelative(employee.identityVerifiedAt ?? null)}.`
      : `Badge ${employee.badgeMarkerId ?? "—"} is visible and verifies Track ${employee.trackId ?? "—"}.`
    : "PPE monitoring active without identity";
