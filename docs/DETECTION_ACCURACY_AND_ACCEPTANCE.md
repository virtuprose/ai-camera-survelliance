...
| Staff visibility | Person tracks, entry/zone activity, optional ArUco employee badge, and track-session identity persistence. | A verified badge remains associated while the same person track is active, including badge occlusion; the binding is retained for up to 30 seconds of track loss and is restored only if the tracker returns the same track ID. Expired or new tracks return to `Unidentified staff member`; PPE never waits for identity and no facial recognition is used. One badge cannot identify two overlapping tracks. | Access-control or badge integration, employee governance, privacy review, camera overlap and multi-camera identity policy. | Single-camera track-session path is software-tested; site/multi-camera design pending. |
